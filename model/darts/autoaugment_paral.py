...
def func_mg(operation, fillcolor, img, magnitude):
    if operation == "shearX":
        img = img.transform(
...
class SubPolicy(object):
    ...
    def __call__(self, img):
        if random.random() < self.p1: img = func_mg(self.operation1, self.fillcolor, img, self.magnitude1)
        if random.random() < self.p2: img = func_mg(self.operation2, self.fillcolor, img, self.magnitude2)
        return img
